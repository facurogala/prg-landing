...
const tshirts: TShirt[] = [
  ...
  },
  {
    ...
    price: 24900,
    image:
      "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/Classic%20remera.jpg-Tm9cxv5iXLnjwmSmg9Bz8zMtmY7Tb8.jpeg",
...
export function TShirtStore() {
  ...
  const handlePurchase = async () => {
    ...
    try {
      ...
      if (data.success) {
        toast.success(
          "Compra realizada con éxito. Usa el botón de WhatsApp para recibir los detalles de la transferencia.",
        )
              ...
              <Card className="bg-black backdrop-blur-md border border-white/10 hover:border-[#DBFF00]/30 transition-all duration-300 flex flex-col h-full">
                ...
                <CardContent className="p-6 flex flex-col flex-grow">
                  <div className="flex-grow">
                    <CardTitle className="text-xl font-semibold mb-2 text-white group-hover:text-[#DBFF00] transition-colors">
                      {tshirt.name}
                    </CardTitle>
                    <CardDescription className="text-gray-400 mb-4">{tshirt.description}</CardDescription>
                  </div>
                  <div className="mt-auto">
                    <p className="text-[#DBFF00] text-lg font-bold mb-4">
                      {tshirt.price.toLocaleString("es-AR", { style: "currency", currency: "ARS" })}
                    </p>
                    <Button
        ...
        {whatsappLink && (
          ...
          >
            <p className="mb-4 text-lg text-gray-300">
              Haz clic en el botón de abajo para abrir WhatsApp y recibir los detalles de tu compra:
            </p>
            <a
              href={whatsappLink}
              target="_blank"
            ...
            <p className="mt-4 text-sm text-gray-400">
              Nota: En el entorno de preview, este enlace no abrirá WhatsApp. En un entorno real, te llevaría
              directamente a la conversación de WhatsApp.
            </p>
          </motion.div>
        )}
      </div>
